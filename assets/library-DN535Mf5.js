import{s as E,c as G,g as M}from"./header-Cl8itn-T.js";import{c as A,b as w,r as k}from"./movie-grid-DhbnrA0Y.js";const q=`<div class="container hero-library__container">
  ...
  `},V=e=>{const n=new Set;return e.forEach(t=>{Array.isArray(t.genres)&&t.genres.forEach(s=>s?.name&&n.add(s.name))}),Array.from(n).sort((t,s)=>t.localeCompare(s))},$=(e,n)=>!n||n==="all"?e:e.filter(t=>Array.isArray(t.genres)&&t.genres.some(s=>s?.name===n)),D=()=>{G(),C(),A();const e=document.querySelector("#library__list"),n=document.querySelector("#genre-filter");if(!e)return;let t=null,s="all";const i=()=>{t&&(t.destroy(),t=null),n&&(n.innerHTML=""),s="all"},y=l=>{if(!n)return;const a=V(l);if(!t){t=x({containerSelector:"#genre-filter",genres:a,initialValue:s,onChange:c=>{s=c,u()}});return}t.updateGenres(a)},u=()=>{const l=M();if(!l.length){i(),T(e);return}y(l);const a=$(l,s);if(!a.length){renderNoResults(e);return}h(e,"grid"),k(a,e)};e.addEventListener("click",l=>{const a=l.target.closest(".movie-card");if(!a)return;const c=a.dataset.id;c&&w(c)}),window.addEventListener("library:changed",u),u()};document.addEventListener("DOMContentLoaded",D);
